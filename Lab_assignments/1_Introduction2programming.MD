...
* Termius: A free SSH client and terminal which you can install and enables scp options. This is what I currently use. [Termius website](https://termius.com/index.html)

##### Windows users
* PuTTY: A terminal which you can install from [the putty website](https://putty.software/)
...
#### Introduction to Sabine, the UH training High Performance Cluster for this course.
...
In order to access the cluster from the terminal, you have to point your computer to the location you want to login to. The address for sabine is `sabine.rcdc.uh.edu `, and to login you specify `cougarnet_accounte@sabine.rcdc.uh.edu`  Your username is the same as your UH username, and when it asks for your password it is the same as your cougarnet credentials. You use the ssh command to login. So, to access your account you would use:
...
Unlike your Mac OS/Windows OS, you don't have GUI access to click or drag and drop. Folders and files are all you have. If you want to access one of these, you need to tell the computer EXACTLY where it is. 
...
Here is an example of what you might see within a directory:

```
Super_awesome_genomics_work/  <-- Your directory, note no spaces!
├── main.sh                   <-- Your "main" script 
├── scripts/                  <-- Modular sub-scripts or functions
│   ├── 01_qc.sh
│   ├── 02_trim.sh
│   ├── 03_align.sh
│   └── 04_quant_or_call.sh
├── test_data/               <-- Downsampled testdata for fast execution and initial troubleshooting
├── results/                 <-- You might find these important
└── logs/                    <-- Output logs
...
This is convenient, because you can use it to call the absolute path with ease, eg: `${DIR}/raw_data/alskdjfaojf.1.fa`. This is often used in scripts, for example to set the output directory into a variable and make a file structure like above.
